...
export function MisTransactionList(props) {
  ...
  function onNextPage() {
    if (misTxList.repudiatedTxs.length === pageState.size) {
  ...
  return (
    <>
      {misTxList.repudiatedTxs !== undefined && misTxList.repudiatedTxs !== null &&
        <Box>
          ...
          <TabPanel sx={{mt: 2}} value={algo} index={0}>
            {!mobile && <MisTable misTxList={misTxList.repudiatedTxs} btcusd={misTxList.btcPrice} algo={algo} />}
            {mobile && <MisTableMobile misTxList={misTxList.repudiatedTxs} btcusd={misTxList.btcPrice} algo={algo} />}
          </TabPanel>
